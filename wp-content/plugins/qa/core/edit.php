...
class QA_Edit {
	...
	function handle_question_editing() {
		...
		if ( empty( $question['post_title'] ) || empty( $question['post_content'] ) )
			wp_die( __( 'Missions must have both a title and a body. Please use your browser\'s back button to edit your mission.', QA_TEXTDOMAIN ) );
		...
		if ( !$question_id ) {
			...
			if ( $dup_id ) {
				wp_die( sprintf( __( 'It seems that this mission was already asked. Click <a href="%s" target="_blank">here</a> to view it, if it is approved by the admin. If title or content of your mission is the same as any of the previous missions, it is regarded as duplicate. Please use your browser\'s back button to edit your mission.', QA_TEXTDOMAIN ), qa_get_url( 'single', $dup_id ) ) );
			}
		}
	...
	function handle_answer_editing() {
		...
		if ( empty( $answer['post_parent'] ) )
			wp_die( __( 'Answer must be associated to a mission.', QA_TEXTDOMAIN ) );
		...
		if ( 'answer' == $post->post_type && 'publish' == $new_status ) {
			...
			if (function_exists('bp_activity_add')) {
				...
				$activity_id = bp_activity_add( array(
					...
					'user_id' => $user_ID,
					'action' => sprintf(__('%s answered mission "<a href="%s">%s</a>"', 'qa'), $current_user->display_name, get_permalink($post->ID), $question->post_title),
