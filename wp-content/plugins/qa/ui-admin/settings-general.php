			...
			<tr>
				<th>
					...
					if ( isset( $options['default_category'] ) )
						...
						$selected = 0;
					$terms = get_terms( 'question_category', apply_filters( 'qa_get_terms_args', array( 'hide_empty' => false ) ) );
					?>
					<select name="default_category">
						<option value='0'><?php _e( 'None', QA_TEXTDOMAIN ) ?></option>
						<?php
						if ( $terms && is_array( $terms ) ) {
							foreach ( $terms as $term ) {
								echo "<option value='". $term->term_id . "' ". selected( $term->term_id, $selected, false ). ">" . $term->name . "</option>";
							}
						}
						?>
					</select>
