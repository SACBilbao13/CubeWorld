 	...
 	$wpdb->show_errors();
  	...
  	echo '<div class="wrap">';
		...
		if (isset($_GET['gotosite']) && $_GET['gotosite'] == '1') {
			echo '<script>';
			echo 'window.location.replace("'.get_bloginfo('url').'");';
			echo '</script>';
			die();
		}
					...
					<?php if (get_option(WPS_OPTIONS_PREFIX.'_use_templates') != "on") { ?>
						...
						if ($ext_rows) {
							...
							foreach ($ext_rows as $row) {
								...
								echo ' <span class="description">'.$row['name'].'</span><br />';
							}
							echo '</td></tr>';
	...
	global $wpdb;
		...
		<form method="post" action=""> 
			...
			if (!$readonly) { ?>
				...
				<tr valign="top"> 
				<td colspan="2"><?php echo '<h2>'.__('Troubleshooting', WPS_TEXT_DOMAIN).'</h2>'; echo __('The following can be disabled if clashes with other WordPress plugins are occuring', WPS_TEXT_DOMAIN); ?>:</td>
				</tr> 
