...
if ($_POST['action'] == 'event_payment') {
	...
	if (is_user_logged_in()) {
		...
		echo sprintf(__("You need to be <a href='%s'>logged in</a> to book events.", WPS_TEXT_DOMAIN), site_url().'/wp-login.php?redirect_to='.urlencode(__wps__pageURL()));
	}
...
if ($_POST['action'] == 'deleteEvent') {
	...
	if (is_user_logged_in()) {
		...
		echo sprintf(__("You need to be <a href='%s'>logged in</a> to delete events.", WPS_TEXT_DOMAIN), site_url().'/wp-login.php?redirect_to='.urlencode(__wps__pageURL()));
	}
